...
print("source_data",source_data.split('\n')[:10])
print("target_data",target_data.split('\n')[:10])

# seq2seq中输入的必须是整数数字，因此要建立一个<中文到整数>的双向转换字典
# 同时增加以下几个特殊字符<PAD> 进行字符不全；<EOS> <GO>用于decoder端的序列，告诉解码器的起始和结束
# <UNK>替代一些未曾出现过的词和低频词
...
def get_encoder_layer(input_data, rnn_size, num_layers, source_sequence_length, source_vocab_size,
    ...
    # LSTMCell和MultiRNNCell中：参数state_is_tuple
    # 如果为True，接受和返回的states是n_tuples，其中 n = len(cells)
    # 如果为False states是concatenated沿着列轴，后者即将弃用

    def get_lstm_cell(rnn_size):
        # rnn_size代表隐藏层神经元个数 返回一个LSTM
        lstm_cell = tf.contrib.rnn.LSTMCell(rnn_size, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=2))
        return lstm_cell
    # 由多个简单rnn_cell构成的RNN cell ，用于构建多层循环神经网络
    cell = tf.contrib.rnn.MultiRNNCell([get_lstm_cell(rnn_size) for _ in range(num_layers)])
    # cell参数代表一个LSTM或GRU的记忆单元
    # encoder_embed_input 是一个list，假设输入了三句话，长度分别为5,10,15；那么sequence_length=[5,10,15]

    # encoder_state：最终的状态，也就是序列中最后一个cell输出的状态；一般情况下states的形状是[batch_size,cell.output_size]
    # 但输入为BasicLSTMCell时，state的形状为[2,batch_size,cell.output_size]其中2对应着LSTM中的cell_state和hidden_state

    # output与states的关系：如果cell为LSTM，那states是个tuple，分别代表Ct和ht；其中ht和outputs中对应的最后一个时刻，即最后一个cell的输出相等；
    # 如果cell为GRU states其实就是ht
